...
def get_inst_groups(**kwargs): 
    ...
    try : 
        ...
        endpoint=f'/{Variable.get("Mabna-Inst-Group-Endpoint")}'
...
def save_2_pg(**kwargs):
    ...
    try : 
        ti = kwargs['ti']
        instgroup_list = ti.xcom_pull( task_ids="etl_mabna_get_instgroups_req")
        logger.info(f"Len of Input data (Ins Groups) : {len(instgroup_list)}")
        logger.info("^-"*25)
        logger.info(f"First Entry : {instgroup_list[0]}")
        logger.info(f"Last Entry : {instgroup_list[-1]}")
        logger.info("^-"*25)

        rows_count = 1
        for inst_group in instgroup_list : 
            try :
                logger.info(f"# {rows_count:5} : {inst_group.get('title','No Title')}")
                # logger.info(i)

                pg_hook = PostgresHook(postgres_conn_id='postgres_lookups')
                inst_group_insert = """INSERT INTO lookups.instrument_groups(id, code, title)
                VALUES(%s, %s,%s);"""

                inst_group_update = """UPDATE lookups.instrument_groups 
                    SET code=%s, title=%s WHERE id=%s;"""

                rows_count+=1
                logger.info(f"Row # {rows_count} is processing ... ")
                ckeck_row_existed = f"select * from lookups.instrument_groups where id = {inst_group.get('id')} "
                ...
                if result != None :
                    ...
                    pg_hook.run(inst_group_update, parameters=(inst_group.get("code",""),
                                                inst_group.get("title",""),
                                                inst_group.get("id")
                                                ))
                else : 
                    pg_hook.run(inst_group_insert, parameters=(inst_group.get("id"),
                                                inst_group.get("code",""),
                                                inst_group.get("title","")
                                                ))
                ...
                raise err
        logger.info(f"Instruments Group List - Len : {len(instgroup_list)}")
        select_rows_affected = """select count(*) as cnt from lookups.instrument_groups where updated_at > now() - interval '1 hour';"""
...
with DAG(
    'ETL-Direct-Mabna-InstrumnetGroup-2-PG',
    ...
    task_get_request = PythonOperator(
        task_id='etl_mabna_get_instgroups_req',
    ...
    task_save_results = PythonOperator(
        task_id='etl_mabna_instgroups_2_pg',
